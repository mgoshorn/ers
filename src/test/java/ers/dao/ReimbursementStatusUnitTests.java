 ...
 */
public class ReimbursementStatusUnitTests {
	...
	public static void setup() throws SQLException {
		...
		while(results.next()) {
			...
			Integer key = results.getInt("REIMB_STATUS_ID");
			String value = results.getString("REIMB_STATUS");
	...
	public void checkEnumValuesMatchDatabase() throws SQLException {
		for(ReimbursementStatus status : ReimbursementStatus.values()) {
			assertEquals(status.toString(), valueMap.get(status.getValue()));
		}
		
		log.trace("valueMap size -> "+valueMap.size()+", ReimbursementStatus values length -> " + ReimbursementStatus.values().length);
		assertTrue(valueMap.size() == ReimbursementStatus.values().length);
	}
